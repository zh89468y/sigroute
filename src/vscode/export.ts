...
/** 把追踪结果转成 Mermaid 流程图 */
export function toMermaid(result: TraceResult, opts: MermaidOptions = {}): string {
  const maxNodes = opts.maxNodes ?? 200;
  const dir = opts.direction ?? 'LR';
  const out: string[] = [`flowchart ${dir}`];
  const classAssign: Record<string, string[]> = { renamed: [], blackbox: [], constant: [], warn: [] };

  const idOf = new Map<TraceNode, string>();
  let count = 0;
  let truncated = 0;

  const ensure = (n: TraceNode): string | null => {
    ...
    if (n.flags.includes('renamed')) classAssign.renamed.push(id);
    else if (n.kind === 'blackbox') classAssign.blackbox.push(id);
    else if (n.kind === 'constant') classAssign.constant.push(id);
    else if (n.flags.includes('partial') || n.flags.includes('limit')) classAssign.warn.push(id);
    return id;
  };

  const walk = (n: TraceNode, parentId: string | null): void => {
    const id = ensure(n);
    if (id === null) return;
    if (parentId) {
  ...
  for (const [cls, ids] of Object.entries(classAssign)) {
    if (ids.length > 0) out.push(`  class ${ids.join(',')} ${cls};`);
  }
...
export function toMarkdown(result: TraceResult): string {
  ...
  const FLAG_MARK: Record<string, string> = {
    renamed: '🔶 **改名**',
    bitselect: '✂ 位选',
    partial: '⚠ 不完整',
    loop: '↺ 已展开',
  ...
  const render = (n: TraceNode, depth: number, isLast: boolean, prefix: string): void => {
