...
export interface IndexCacheFile {
  ...
  files: FileStamp[];
  /**
   * 被 `include 引用的文件内容（通常很小）。
   * 带上它是为了让缓存接管之后的"保存时增量更新"仍能内联 include。
   */
...
function decode(v: unknown): any {
  if (Array.isArray(v)) return v.map(decode);
  if (v && typeof v === 'object') {
    const o = v as Record<string, unknown>;
    if (Array.isArray(o.__map)) {
      const m = new Map();
      for (const pair of o.__map as [string, unknown][]) m.set(pair[0], decode(pair[1]));
      return m;
    }
    if (Array.isArray(o.__set)) {
      const s = new Set();
      for (const x of o.__set as unknown[]) s.add(decode(x));
      return s;
    }
    const out: Record<string, unknown> = {};
...
export function serializeIndex(index: WorkspaceIndex): unknown {
  return encode(index);
}

export function deserializeIndex(data: unknown): WorkspaceIndex | null {
  try {
    const idx = decode(data) as WorkspaceIndex;
    if (!idx || !(idx.modules instanceof Map) || !(idx.fileModules instanceof Map)) return null;
    if (!(idx.instantiations instanceof Map)) return null;
    if (!Array.isArray(idx.topCandidates)) return null;
    if (!idx.stats || typeof idx.stats.fileCount !== 'number') return null;
    return idx;
  } catch {
    return null;
  }
...
export function optionsFingerprint(opts: {
  honorIfdef: boolean;
...
export function sameFileSet(cached: FileStamp[], current: FileStamp[]): boolean {
  if (cached.length !== current.length) return false;
  const map = new Map(cached.map((f) => [f.path, f]));
  for (const c of current) {
    const old = map.get(c.path);
    if (!old) return false;
    if (old.size !== c.size) return false;
    // 时间戳用毫秒比较，容差 1ms 规避文件系统精度差异
    if (Math.abs(old.mtimeMs - c.mtimeMs) > 1) return false;
  }
  return true;
}
