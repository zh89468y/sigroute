...
import type { ModuleDecl, PortDirection, WorkspaceIndex } from './types';

/** 等价类里的一个成员：某个模块里的某个名字 */
export interface NetMember {
  module: string;
  net: string;
  kind: 'port' | 'signal';
  direction: PortDirection | null;
  /** 该模块被例化的次数（为 1 时说明这条别名只在一个实例路径上成立） */
  instances: number;
}

/** 只依赖索引的这两个能力，避免与 WorkspaceIndexer 形成循环依赖 */
export interface ModuleLookup {
  getModule(name: string): ModuleDecl | undefined;
}

function key(moduleName: string, net: string): string {
...
export class NetClasses {
  ...
  /** 网络类被裁剪过的提示（超大规模设计时保护内存） */
  readonly truncated: boolean;
  /** 因"同一子端口被多个父层网络连接"而跳过的等价边数（诊断用） */
  private readonly hubPorts: number;
  ...
  classIdOf(moduleName: string, net: string): string | undefined {
   ...
   */
  aliasesOf(moduleName: string, net: string): NetMember[] {
  ...
  otherAliases(moduleName: string, net: string, limit = 12): NetMember[] {
    const all = this.aliasesOf(moduleName, net);
    const rest = all.filter((m) => !(m.module === moduleName && m.net === net));
    rest.sort((a, b) => {
      if ((a.kind === 'port') !== (b.kind === 'port')) return a.kind === 'port' ? -1 : 1;
  ...
  /** 统计信息（用于状态栏/诊断） */
  stats(): { classes: number; nets: number; aliasedNets: number; hubPorts: number } {
    let nets = 0;
    let aliased = 0;
    for (const list of this.members.values()) {
      nets += list.length;
      if (list.length > 1) aliased += list.length;
    }
...
/** 空实现：索引尚未构建时使用 */
export function emptyNetClasses(): NetClasses {
  return new NetClasses(new Map(), new Map());
}

export interface BuildNetOptions {
  /** 单个等价类的成员上限（防止时钟/复位这类网络把内存吃满） */
 ...
 */
export function buildNetClasses(
  index: WorkspaceIndex,
  lookup: ModuleLookup,
  opts: BuildNetOptions = {},
): NetClasses {
  ...
  const find = (x: string): string => {
    let r = x;
    let guard = 0;
    while (parent.get(r) !== undefined && parent.get(r) !== r && guard++ < 10_000) r = parent.get(r)!;
    // 路径压缩
    let cur = x;
  ...
  const edges: { father: string; child: string }[] = [];
  const childFathers = new Map<string, Set<string>>();
  for (const list of index.modules.values()) {
    for (const mod of list) {
      if (mod.declOnly) continue; // 声明文件只提供端口方向，不构成真实连接
      for (const inst of mod.instances) {
        const sub = lookup.getModule(inst.moduleType);
        if (!sub || sub.declOnly) continue;
        const subPorts = new Set(sub.ports.map((p) => p.name));
        for (const conn of inst.connections) {
          if (conn.kind !== 'net' || !conn.primaryNet) continue;
          const port =
            conn.port ?? (conn.portIndex !== null ? sub.portOrder[conn.portIndex] ?? null : null);
          if (!port || !subPorts.has(port)) continue;
   ...
   */
  let hubPorts = 0;
  for (const set of childFathers.values()) if (set.size > 1) hubPorts++;
  for (const e of edges) {
    if ((childFathers.get(e.child)?.size ?? 1) > 1) continue;
  ...
  const addMember = (k: string): void => {
    ...
    if (seen.get(root)!.has(dedupKey)) return;
  ...
  for (const k of parent.keys()) addMember(k);
