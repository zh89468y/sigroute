...
export interface InlineResult {
  ...
  /** 成功内联的数量 */
  count: number;
  /** 未能内联的 include 名（解析不到 / 内容含 module / 嵌套） */
  skipped: string[];
}
 ...
 */
function maskComments(line: string, inBlock: boolean): { mask: string; inBlock: boolean } {
  let out = '';
  let i = 0;
  let block = inBlock;
  while (i < line.length) {
...
export interface InlineOptions {
  /** 解析 include 名 → 文件与内容 */
  resolve: (name: string, fromFile: string) => IncludeResolveResult | null;
  /** 递归深度上限（本实现的嵌套支持仅到一层，这里保留给未来使用） */
  maxDepth?: number;
  /** 单个被内联文件的大小上限（防止把巨型头文件塞进来） */
  maxBytes?: number;
}
 ...
 */
export function inlineIncludes(text: string, file: string, opts: InlineOptions): InlineResult {
  ...
  if (count === 0) return { text, segments: [], count: 0, skipped };
 ...
 */
export function createOffsetMapper(segments: InlineSegment[]): (offset: number) => number {
  if (segments.length === 0) return (o) => o;
  return (offset: number): number => {
    let shift = 0;
    for (const seg of segments) {
      if (offset < seg.start) break;
      if (offset < seg.end) return seg.origStart;
 ...
 */
export function remapModulePositions(
  mod: ModuleDecl,
   ...
   */
  const remap = (offset: number | null | undefined, line: number | undefined): { off: number; line: number } => {
  ...
  for (const inst of mod.instances) {
    ...
    for (const c of inst.connections) {
      ...
      if (c.netOffset !== null) c.netOffset = remap(c.netOffset, undefined).off;
    }
  }
