...
import { collectMacros, calcWidth, parseFile } from './parser';
import type { IncludeResolveResult } from './inline';
import { buildNetClasses, emptyNetClasses } from './nets';
import type { NetClasses, NetMember } from './nets';
import type { ModuleDecl, PortDecl, WorkspaceIndex, Instance } from './types';
...
export interface FileProvider {
  ...
  /** 文件是否存在（用于 `include 解析） */
  exists?(path: string): Promise<boolean>;
  /** 解析 `include 相对路径 */
  resolveInclude?(fromFile: string, includeName: string): Promise<string | null>;
  /**
   * 返回"仅用于端口声明"的辅助文件（IP 的 .veo/.vho 例化模板等）。
   * 它们只用来补齐黑盒 IP 的端口方向，不会覆盖工程里的真实实现。
   */
  listAuxFiles?(): Promise<string[]>;
  /** 文件时间戳/大小，用于索引缓存校验；不实现则不启用缓存 */
...
export interface BuildStats {
  /** 内联的 include 数量 */
  inlined: number;
  /** 未能内联的 include（名称+原因），最多保留 20 条 */
  skippedIncludes: string[];
  /** 从辅助声明文件补到的模块数（消除黑盒方向未知） */
  declOnlyModules: number;
}
...
export class WorkspaceIndexer {
  ...
  private fileText = new Map<string, string>();
  /** include 名（小写 basename） -> 文件与内容，用于 O(1) 解析 */
  private includeIndex = new Map<string, { file: string; text: string }>();
  /** 文件 -> 该文件里的模块例化出的模块类型集合（增量维护例化反查表用） */
  private instByFile = new Map<string, Set<string>>();
  /** 文件 -> 该文件贡献的例化数量（增量维护统计用） */
  private instCountByFile = new Map<string, number>();
  /** 网络等价类 */
  private nets: NetClasses = emptyNetClasses();
  private lastBuild: BuildStats = { inlined: 0, skippedIncludes: [], declOnlyModules: 0 };
  /**
   * 被 `include 引用的文件内容（体积小、数量少）。
   * 单独留一份是为了让索引缓存接管之后，后续"保存时增量更新"仍然能内联 include ——
   * 否则缓存命中后的第一次保存会悄悄退化成"不内联"。
   */
  private includeTexts = new Map<string, string>();

  get current(): WorkspaceIndex {
    return this.index;
  }
  ...
  get netClasses(): NetClasses {
    return this.nets;
  }

  setOptions(opts: Partial<IndexerOptions>): void {
    this.options = { ...this.options, ...opts };
  }

  // ------------------------------------------------------------------ 构建

  async build(provider: FileProvider, report?: ProgressReporter): Promise<WorkspaceIndex> {
    const t0 = Date.now();
    report?.('扫描源文件…');
    ...
    const readAll = async (list: string[], onOne?: (f: string) => void): Promise<void> => {
      const BATCH = 32;
    ...
    await readAll(files, (f) => {
      ...
      const res = collectMacros(text, defines);
      for (const inc of res.includes) includes.add(inc);
    });
    ...
    for (const f of includeFiles) {
      const text = texts.get(f);
      if (text !== undefined) this.includeTexts.set(normalizePath(f), text);
    }

    for (const u of this.options.undefines) defines.delete(u);
    ...
    const instantiations = new Map<string, { parentModule: string; parentFile: string; instance: Instance }[]>();
    let instanceCount = 0;
    let inlined = 0;
    const skippedIncludes = new Set<string>();
    ...
    for (let i = 0; i < allFiles.length; i += BATCH) {
      ...
      const results = await Promise.all(
        slice.map(async (f) => {
          ...
          try {
            return parseFile(text, f, parseOpts);
          } catch (err) {
            return {
              modules: [],
              warnings: [`解析失败：${(err as Error).message}`],
            };
          }
        }),
      );
      ...
      for (let k = 0; k < slice.length; k++) {
        ...
        for (const mod of res.modules) {
          if (!modules.has(mod.name)) modules.set(mod.name, []);
          modules.get(mod.name)!.push(mod);
          names.push(mod.name);

          for (const inst of mod.instances) {
            if (!instantiations.has(inst.moduleType)) instantiations.set(inst.moduleType, []);
        ...
        if (names.length > 0) fileModules.set(f, names);
        if (types.size > 0) this.instByFile.set(f, types);
        if (fileInstances > 0) this.instCountByFile.set(f, fileInstances);

        inlined += res.inlined ?? 0;
        for (const s of res.skippedIncludes ?? []) {
          if (skippedIncludes.size < 20) skippedIncludes.add(s);
        }
      }
   ...
   */
  private async addDeclOnlyModules(
    modules: Map<string, ModuleDecl[]>,
    provider: FileProvider,
  ): Promise<number> {
    if (!provider.listAuxFiles) return 0;
    let auxFiles: string[] = [];
    try {
    ...
    for (const f of auxFiles) {
      ...
      for (const mod of parsed.modules) {
        ...
        if (mod.ports.length === 0) continue;
  ...
  private rebuildIncludeIndex(texts: Map<string, string>): void {
    for (const [p, t] of texts) {
      const base = p.split('/').pop()?.toLowerCase();
      if (!base) continue;
      if (!this.includeIndex.has(base)) this.includeIndex.set(base, { file: p, text: t });
    }
  }

  /** 供解析器内联 `include 使用 */
  private makeIncludeResolver(): (name: string, fromFile: string) => IncludeResolveResult | null {
    return (name: string, fromFile: string): IncludeResolveResult | null => {
      const dir = normalizePath(fromFile).replace(/\/[^/]*$/, '/');
      const direct = normalizePath(dir + name);
      const dt = this.fileText.get(direct);
      if (dt !== undefined) return { file: direct, text: dt };
  ...
  private computeTopCandidates(
    modules: Map<string, ModuleDecl[]>,
    instantiations: Map<string, unknown>,
  ): string[] {
    const out: string[] = [];
    for (const [name, list] of modules) {
      if (instantiations.has(name)) continue;
      // 只用于声明方向的辅助文件不算顶层候选
      if (list.length > 0 && list.every((m) => m.declOnly)) continue;
      out.push(name);
    }
    out.sort();
    return out;
  ...
  private async resolveIncludes(
    ...
    for (const inc of includes) {
      ...
      if (hit) out.add(normalizePath(hit.file));
      else missing.push(inc);
    }
  ...
  /** 撤掉某个文件贡献的模块定义与例化记录 */
  private dropFileRecords(f: string): void {
    const oldNames = this.index.fileModules.get(f) ?? [];
    for (const name of oldNames) {
      const list = this.index.modules.get(name);
      if (!list) continue;
      const kept = list.filter((m) => m.file !== f);
      if (kept.length === 0) this.index.modules.delete(name);
      else this.index.modules.set(name, kept);
    }
    ...
    if (types) {
      for (const t of types) {
        ...
        if (kept.length === 0) this.index.instantiations.delete(t);
        else this.index.instantiations.set(t, kept);
      }
  ...
  /** 把某个模块的例化登记进反查表 */
  private addModuleInstances(mod: ModuleDecl, f: string): void {
    if (mod.instances.length === 0) return;
  ...
  updateFile(file: string, text: string): void {
    ...
    const names: string[] = [];
    for (const mod of parsed.modules) {
      if (!this.index.modules.has(mod.name)) this.index.modules.set(mod.name, []);
      this.index.modules.get(mod.name)!.push(mod);
      names.push(mod.name);
      this.addModuleInstances(mod, f);
    }
    if (names.length > 0) this.index.fileModules.set(f, names);
   ...
   */
  adopt(index: WorkspaceIndex, fileText?: Map<string, string>): void {
    this.index = index;
    if (fileText) {
      for (const [k, v] of fileText) this.includeTexts.set(normalizePath(k), v);
    }
    if (this.includeTexts.size > 0) this.rebuildIncludeIndex(this.includeTexts);
    this.instByFile.clear();
    this.instCountByFile.clear();
    for (const list of index.modules.values()) {
      for (const mod of list) {
        if (mod.declOnly) continue;
        for (const inst of mod.instances) {
    ...
    if (fileText) {
      for (const [k, v] of fileText) this.fileText.set(normalizePath(k), v);
    }
    this.rebuildTopCandidates();
    this.rebuildNets();
  }
  ...
  get cachedIncludeTexts(): Map<string, string> {
  ...
  getModule(name: string): ModuleDecl | undefined {
    const list = this.index.modules.get(name);
    if (!list || list.length === 0) return undefined;
    if (list.length === 1) return list[0];
    let best: ModuleDecl | undefined;
    for (const m of list) {
      if (m.declOnly) continue;
      if (!best || m.instances.length > best.instances.length) best = m;
    }
    return best ?? list[0];
  }

  getModules(name: string): ModuleDecl[] {
  ...
  /** 该模块是否只有"声明"（IP 例化模板） */
  isDeclOnly(name: string): boolean {
    const list = this.index.modules.get(name);
    return !!list && list.length > 0 && list.every((m) => m.declOnly);
  }

  getInstantiations(name: string) {
    return this.index.instantiations.get(name) ?? [];
  }

  getPort(mod: ModuleDecl, portName: string): PortDecl | undefined {
    return mod.ports.find((p) => p.name === portName);
  }

  /** 同一物理网络的其它名字（改名反查） */
  netAliases(moduleName: string, net: string, limit = 12): NetMember[] {
    return this.nets.otherAliases(moduleName, net, limit);
  }

  /** 该网络所属等价类里的全部成员（含自身） */
  netMembers(moduleName: string, net: string): NetMember[] {
    return this.nets.aliasesOf(moduleName, net);
  }

  /** 光标字符偏移落在哪个模块内 */
  findModuleAt(file: string, offset: number): ModuleDecl | undefined {
    const f = normalizePath(file);
   ...
   */
  findModulesDeclaring(netName: string): ModuleDecl[] {
  ...
  /** 某个网络名是否可能对应模块内声明的信号 */
  lookupSignal(mod: ModuleDecl, netName: string): 'port' | 'signal' | 'unknown' {
    if (mod.ports.some((p) => p.name === netName)) return 'port';
    if (mod.signals.has(netName)) return 'signal';
    return 'unknown';
  }

  /** 调试用：模块的端口位宽摘要 */
  describePort(mod: ModuleDecl, portName: string): string {
    const p = this.getPort(mod, portName);
    if (!p) return '';
    const dir = p.direction ?? '?';
    let w = '';
    if (p.msb !== null && p.lsb !== null) w = ` [${p.msb}:${p.lsb}]`;
    const width = calcWidth(p.msb, p.lsb);
 ...
 */
export function looksNonTextual(text: string): string | null {
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (c === 0) return '含 NUL 字节（二进制文件）';
    if (c < 32 && c !== 9 && c !== 10 && c !== 13 && c !== 12) ctrl++;
    else if (c >= 0x2000) wide++;
  }
  if (ctrl / n > 0.02) return `控制字符过多（${Math.round((ctrl / n) * 100)}%）`;
  if (wide / n > 0.3) return `非 ASCII 字符过多（${Math.round((wide / n) * 100)}%，疑似加密或二进制）`;
  return null;
}
