...
import type { Instance, ModuleDecl, WorkspaceIndex } from './types';
...
export interface HierarchyLookup {
  ...
  getInstantiations(name: string): { parentModule: string; parentFile: string; instance: Instance }[];
}

export interface HierarchyOptions {
  /** 最多返回多少条路径（同一模块被多处例化时会有多条） */
  maxPaths?: number;
  /** 向上追溯的最大层数 */
  maxDepth?: number;
}
 ...
 */
export function hierarchyPaths(
  ...
  const climb = (
    ...
    visited: Set<string>,
    budget: { left: number },
  ): { paths: HierarchyPath[]; truncated: boolean } => {
    const mod = lookup.getModule(name);
    const base: HierarchyStep = {
      moduleName: name,
      file: mod?.file ?? '',
    ...
    for (const site of sites) {
      ...
      if (visited.has(site.parentModule)) continue;

      const nextVisited = new Set(visited);
      nextVisited.add(name);
      const parents = climb(site.parentModule, depth + 1, nextVisited, budget);
      if (parents.truncated) truncated = true;
...
/** 取最"像顶层"的那条路径，用于一行显示 */
export function primaryPath(paths: HierarchyPath[]): HierarchyPath | undefined {
