...
export type TraceFlag =
  | 'renamed'        // 跨模块时名字变了
  | 'bitselect'      // 只用了信号的一部分位
  | 'partial'        // 表达式/拼接，静态无法完整追踪
  | 'loop'           // 链路成环或已展开过
  | 'limit'          // 达到深度上限
  | 'constant'       // 常量注入
  | 'unconnected'    // 悬空
  | 'generate'       // 位于 generate 块内（实例名可能带索引）
  | 'always'         // 由时序/过程块驱动
  | 'intra'          // 模块内部的数据流（assign / 过程块），非跨模块跳转
  | 'budget'         // 因节点预算被截断
  | 'cross'          // 一次跨模块跳跃（树视图可按此过滤）
  | 'more'           // 被分支上限收起的分支，可点击继续展开
  | 'summary'        // 链路摘要节点（过滤时始终保留）
  | 'ambiguous';     // 同一模块被多处例化，存在多个可能路径
...
export interface TraceNode {
  ...
  /** 被收起的分支数 */
  pendingCount?: number;
  /** 同一物理网络的其它名字（跨模块别名），最多 8 条 */
  aliases?: string[];
  /** 分支方向（只标在"上游 / 下游"分组节点上），供脑图区分颜色 */
  branch?: 'up' | 'down';
}
...
export interface TraceOptions {
  ...
  maxDepth: number;
  /** 节点总数预算，防止超宽扇出把 TreeView / Webview 拖死 */
  maxNodes?: number;
  /** 单个节点的子分支上限，超出只保留前 N 个并提示 */
  maxChildren?: number;
  /**
   * 已经"继续展开"过的节点：键 → 次数。
   * 每次点击把该节点的分支上限放宽 4 倍，可以反复点，直到全部展开。
   */
  expanded?: Record<string, number>;
  /**
   * 发起追踪的位置（光标处 / 悬停链接处 / 框图连线处）。
   * 根节点会指向这里，而不是信号的声明处 ——
   * "我当时是在这里看到它的"，点回去应该回到原处（声明位置放进悬停提示里）。
   */
 ...
 * 框图用的模块节点。
 *
 * 刻意只到"模块"粒度：模块内部的 assign / always 传播不画成边，
 * 因为用户要看的是"信号穿越了哪些模块"，而不是模块内部的每个中间变量。
 */
export interface SignalGraphNode {
  /** 唯一 id（当前为模块名；同一模块的多个实例会合并为一个节点） */
  id: string;
  moduleName: string;
  /** 是否信号链的起点模块 */
  isStart: boolean;
  ...
  blackbox: boolean;
  /**
   * 真实例化信息：本模块是被哪个模块（container）通过哪个实例（instanceName）例化出来的。
   * 框图用它还原"包含 / 同层 / 跨层"：同一 container 下的节点就是同一层的兄弟，
   * 边的两端 container 不同即跨层次。
   */
  container?: string;
  instanceName?: string;
  /**
   * 该模块在链路上出现过的全部"（父模块, 实例名）"组合（首个为主，最多 4 组）。
   * 同一模块被多处例化时，这里能给出"它在不同父模块里分别叫什么"，
   * 层次框（同层兄弟）就是按这个分组的。
   */
  instances?: { container: string; instanceName: string }[];
}
...
export interface SignalGraphEdge {
  ...
  flags: TraceFlag[];
  /**
   * 这条连接距选中信号的跳数（1 = 与信号直接相连）。
   *
   * 注意必须用"信号传播层数"而不是"图上的列距离"：
   * data_path 与 mem_ctrl 在图上直接相邻，但对某根具体信号来说，
   * 可能要逆向追溯 7 层才到达 mem_ctrl —— 后者才是用户要的"关联强度"。
   */
  hop: number;
  /** 合并了多少条同向连接（多通道总线常出现一对模块间 4~8 条连接） */
  count?: number;
  /** 合并前的全部信号名（用于悬停时展示完整清单） */
  allNets?: string[];
  /** 证据位置（例化处 / 连接处），用于点击跳转 */
  file?: string;
  line?: number;
...
export interface SignalGraph {
  ...
  startModule: string;
  signalName: string;
  stats: { nodes: number; edges: number; renames: number };
}
...
interface Ctx {
  ...
  modules: string[];
  moduleSeen: Set<string>;
  /** 用户点击"继续展开"过的节点键 → 次数 */
  expanded: Map<string, number>;
  // ---- 模块级图（框图数据），在每次"跨模块跳转"处直接记录 ----
  gNodeMap: Map<string, SignalGraphNode>;
  gEdgeMap: Map<string, SignalGraphEdge>;
  startModule: string;
}

/** 取（或创建）框图节点 */
function gnode(ctx: Ctx, moduleName: string): SignalGraphNode {
...
function noteInstance(ctx: Ctx, from: string, to: string, instanceName?: string): void {
  if (!instanceName || from === to) return;
  const node = ctx.gNodeMap.get(to);
  if (!node) return;
  const list = node.instances ?? [];
  if (list.some((x) => x.container === from && x.instanceName === instanceName)) return;
  if (list.length >= MAX_INSTANCES_PER_NODE) return;
...
function addEdge(
  ...
  if (!src.outNets.includes(fromNet)) src.outNets.push(fromNet);
  if (from === to) {
    // 自环（信号在同一模块类型内绕回）：不进图，但把信号登记下来
    if (!src.inNets.includes(toNet)) src.inNets.push(toNet);
    return id;
  }
  const dst = gnode(ctx, to);
  if (!dst.inNets.includes(toNet)) dst.inNets.push(toNet);
  noteInstance(ctx, from, to, meta.instanceName);
  for (const f of meta.flags) {
    if (!src.flags.includes(f)) src.flags.push(f);
    if (!dst.flags.includes(f)) dst.flags.push(f);
  }

  if (ctx.gEdgeMap.has(id)) return id;
  ctx.gEdgeMap.set(id, {
    id,
    from,
    to,
...
function buildGraph(ctx: Ctx, signalName: string): SignalGraph {
  ...
  for (const n of nodes) {
    if (n.isStart) n.hop = 0;
  }
  for (const e of edges) {
    const a = byId.get(e.from);
    const b = byId.get(e.to);
    if (a && e.hop < a.hop) a.hop = e.hop;
    if (b && e.hop < b.hop) b.hop = e.hop;
  }
  for (const n of nodes) {
    if (!Number.isFinite(n.hop)) n.hop = 0;
  }
 ...
 */
function capChildren(ctx: Ctx, out: TraceNode[], key: string): TraceNode[] {
  ...
  return [
    ...shown,
    {
      ...
      tooltip:
        `该节点下共有 ${out.length} 个分支，当前显示前 ${cap} 个。\n` +
...
function budgetNode(ctx: Ctx): TraceNode {
  return {
    kind: 'note',
...
function dirOf(mod: ModuleDecl, net: string): PortDirection | null {
  return mod.ports.find((p) => p.name === net)?.direction ?? null;
}

function portLine(mod: ModuleDecl, port: string): number {
  return mod.ports.find((p) => p.name === port)?.line ?? mod.headerLine;
}

/** 端口名在模块文件中的精确偏移，用于跳转时高亮到具体标识符 */
function portOffset(mod: ModuleDecl, port: string): number | undefined {
 ...
 */
function netLoc(mod: ModuleDecl, net: string): { file: string; line: number; offset?: number } {
  const port = mod.ports.find((p) => p.name === net);
  if (port) return { file: mod.file, line: port.line, offset: port.offset };
...
function upKey(mod: ModuleDecl, net: string, depth: number): string {
  return `${mod.name}::${net}::up::${depth}`;
}

function downKey(mod: ModuleDecl, net: string, depth: number): string {
 ...
 */
function aliasTexts(ctx: Ctx, moduleName: string, net: string, limit = 8): string[] | undefined {
...
function summaryNode(ctx: Ctx, startModule: string, net: string, dir: PortDirection | null): TraceNode {
  ...
  if (s.blackboxes > 0) bits.push(`黑盒 ${s.blackboxes} 处`);
  if (s.uncertainties > 0) bits.push(`不确定 ${s.uncertainties} 处`);
...
function resolveConnPort(indexer: WorkspaceIndexer, instModuleType: string, conn: Connection): string | null {
  if (conn.port) return conn.port;
  if (conn.portIndex === null) return null;
  const sub = indexer.getModule(instModuleType);
  return sub?.portOrder[conn.portIndex] ?? null;
}

function findConnForPort(indexer: WorkspaceIndexer, instModuleType: string, conns: Connection[], port: string): Connection | undefined {
...
export function traceSignal(
  ...
  const root: TraceNode = {
    ...
    tooltip:
      `起点：模块 ${mod.name} 中的 ${net}${dir ? `（${dir} 端口）` : ''}\n` +
...
function touch(ctx: Ctx, moduleName: string): void {
...
function countLeaves(nodes: TraceNode[]): number {
  let n = 0;
  const walk = (list: TraceNode[]): void => {
    for (const x of list) {
      n++;
      walk(x.children);
    }
...
function upNodes(ctx: Ctx, mod: ModuleDecl, net: string, depth: number): TraceNode[] {
  const out: TraceNode[] = [];
  ...
  if (dir === 'input' || dir === 'inout') {
    ...
    if (sites.length === 0) {
      ...
      if (sites.length > 1) ctx.stats.uncertainties++;
      for (const site of sites) {
        ...
        if (!parentMod || !conn.primaryNet) continue;

        const flags: TraceFlag[] = [];
        if (conn.primaryNet !== net) flags.push('renamed');
        if (conn.kind === 'expression') flags.push('partial');
        if (conn.bitSelect) flags.push('bitselect');
        if (site.instance.inGenerate) flags.push('generate');
        if (sites.length > 1) flags.push('ambiguous');
        if (flags.includes('renamed')) ctx.stats.renames++;
        if (flags.includes('partial')) ctx.stats.uncertainties++;
  ...
  for (const inst of mod.instances) {
    for (const conn of inst.connections) {
      if (!conn.nets.includes(net)) continue;
      ...
      if (sd !== 'output' && sd !== 'inout') continue;

      const flags: TraceFlag[] = [];
      if (pname !== net) flags.push('renamed');
      if (inst.inGenerate) flags.push('generate');
      if (conn.kind === 'expression') flags.push('partial');
      if (flags.includes('renamed')) ctx.stats.renames++;
  ...
  for (const ab of mod.alwaysBlocks) {
    ...
    let anyLine: number | undefined;
    let anyOffset: number | undefined;

    for (const st of ab.statements) {
      if (!st.lhs.includes(net)) continue;
    ...
    out.push({
      ...
      label: ab.isSequential
        ...
        : 'always @(*)',
      description:
        `${mod.name} · ${fname(mod.file)}:${targetLine + 1}` +
        (located ? ` · 赋值给 ${net}` : ''),
      tooltip:
  ...
  if (out.length === 0) {
    ...
    out.push({
      ...
      tooltip:
        `可能原因：由原语/IP 驱动、由未解析的语法结构产生、或名称拼写不一致。\n` +
        `声明位置：${fname(at.file)}:${at.line + 1}`,
      file: at.file,
      line: at.line,
      offset: at.offset,
...
function downNodes(ctx: Ctx, mod: ModuleDecl, net: string, depth: number): TraceNode[] {
  const out: TraceNode[] = [];
  if (depth > ctx.maxDepth) {
    return [limitNode(net, depth)];
  }
  ...
  for (const ab of mod.alwaysBlocks) {
     ...
     */
    const targets = new Map<string, { offset: number; line: number }>();
    for (const st of ab.statements) {
      if (!st.reads.includes(net)) continue;
      st.lhs.forEach((l, i) => {
        if (l !== net && !targets.has(l)) targets.set(l, { offset: st.lhsOffsets[i] ?? -1, line: st.line });
      });
    }
    ...
    if (targets.size === 0 && ab.readNets.includes(net)) {
      ...
      out.push({
        ...
        description:
          `${mod.name} · ${fname(mod.file)}:${(ref?.line ?? ab.line) + 1}` +
          ` · 条件/索引中的 ${net}`,
        edgeText: '过程块内作为条件/索引使用',
        tooltip:
          '该信号在此过程块中以条件 / 索引的形式参与运算。\n' +
          (ref
  ...
  if (out.length === 0) {
    ...
    out.push({
      ...
      tooltip:
        `可能原因：只在该模块内被读取但未跨模块、或负载在未解析的语法结构中。\n` +
        `声明位置：${fname(at.file)}:${at.line + 1}`,
      file: at.file,
      line: at.line,
      offset: at.offset,
...
function limitNode(net: string, depth: number): TraceNode {
  return {
    kind: 'note',
...
export function resolveStartPoint(
  ...
  if (hits.length > 1) {
    ...
    let best = hits[0];
    for (const h of hits) {
      if (h.sub.instances.length > best.sub.instances.length) best = h;
    }
    return {
      module: best.sub,
      net,
  ...
  if (decls.length > 1) {
    ...
    const preferred =
      decls.find((m) => subNames.has(m.name)) ??
    ...
    return {
      ...
      net,
      note:
        `该名称在 ${decls.length} 个模块中出现，已选择 ${preferred.name}` +
