...
export type TokType = (typeof TokType)[keyof typeof TokType];
...
export function lex(text: string, opts: LexOptions): LexResult {
  const tokens: Token[] = [];
  const localDefines: string[] = [];
  const localUndefines: string[] = [];
  ...
  const isActive = (): boolean => (opts.honorIfdef ? condStack.every((f) => f.active) : true);

  const push = (type: TokType, value: string, start: number, end: number, ln: number, active: boolean): void => {
    tokens.push({ type, value, start, end, line: ln, inactive: !active });
  };

  /** 跳过空白与注释，返回是否遇到换行（用于行计数在调用方统一处理） */
  const skipWhiteAndComment = (): void => {
    while (i < len) {
      const ch = text[i];
      if (ch === '\n') {
        line++;
  ...
  while (i < len) {
    ...
    if (ch === '`') {
      ...
      push(TokType.Directive, name, start, i, startLine, active);
      continue;
    }
    ...
    if (ch === '"') {
      ...
      i++;
      push(TokType.String, text.slice(start, i), start, i, startLine, active);
      continue;
    }
    ...
    if (ch === '$') {
      ...
      i++;
      while (i < len && isIdentChar(text[i])) i++;
      push(TokType.SysTask, text.slice(start, i), start, i, line, active);
      continue;
    }
    ...
    if (isDigit(ch) || (ch === "'" && /[sSbBoOdDhH]/.test(text[i + 1] ?? ''))) {
      const start = i;
      while (i < len && (isDigit(text[i]) || text[i] === '_')) i++;
      if (text[i] === "'") {
        i++;
        if (/[sS]/.test(text[i] ?? '')) i++;
        if (/[bBoOdDhH]/.test(text[i] ?? '')) i++;
        while (i < len && /[0-9a-fA-FxXzZ?_]/.test(text[i])) i++;
      } else {
        // 实数 / 指数
      ...
      push(TokType.Number, text.slice(start, i), start, i, line, active);
      continue;
    }
    ...
    if (isIdentStart(ch)) {
      ...
      push(TokType.Ident, value, start, i, line, active);
      continue;
    }
    ...
    if (PUNCT3.includes(three)) {
      push(TokType.Punct, three, i, i + 3, line, active);
      i += 3;
      continue;
    }
    const two = text.slice(i, i + 2);
    if (PUNCT2.includes(two)) {
      push(TokType.Punct, two, i, i + 2, line, active);
      i += 2;
      continue;
    }

    // ---------- 单字符 ----------
    push(TokType.Punct, ch, i, i + 1, line, active);
    i++;
  }
...
export function isPlainIdent(t: Token | undefined | null): t is Token {
  return !!t && t.type === TokType.Ident && !KEYWORDS.has(t.value);
}

export function isPunct(t: Token | undefined | null, v: string): boolean {
  return !!t && t.type === TokType.Punct && t.value === v;
}

export function isIdent(t: Token | undefined | null, v: string): boolean {
  return !!t && t.type === TokType.Ident && t.value === v;
}
