...
exports.DEFAULT_HOVER_OPTIONS = exports.HOVER_SECTION_LABEL = exports.HOVER_SECTIONS = void 0;
exports.isHoverSection = isHoverSection;
exports.renderSignalHoverMarkdown = renderSignalHoverMarkdown;
const describe_1 = require("./describe");
const hierarchy_1 = require("./hierarchy");
/** 可显示的小节（顺序即默认显示顺序） */
exports.HOVER_SECTIONS = ['decl', 'drivers', 'loads', 'parent', 'path', 'aliases'];
exports.HOVER_SECTION_LABEL = {
    decl: '声明（模块 / 位置）',
    drivers: '驱动源（在哪个模块产生）',
    loads: '负载（流向哪个模块）',
...
exports.DEFAULT_HOVER_OPTIONS = {
    sections: [...exports.HOVER_SECTIONS],
    maxRefs: 12,
    maxLines: 40,
    declSource: true,
};
function isHoverSection(v) {
    return exports.HOVER_SECTIONS.includes(v);
}
// ------------------------------------------------------------------ 小工具
/** 生成一个可点击的命令链接 */
function cmdLink(command, args, label) {
    const q = encodeURIComponent(JSON.stringify(args));
    return `[${label}](command:${command}?${q})`;
}
/** 位置文本 → 可点击跳转链接 */
function locLink(file, line, offset, name) {
    return cmdLink('sigroute.reveal', [file, line, offset, name], escapeLinkText((0, describe_1.locText)(file, line)));
}
...
function declBadge(d) {
    const bits = [];
    if (d.decl.direction)
        bits.push(d.decl.direction);
    else if (d.decl.signalKind && d.decl.kind === 'signal')
        bits.push(d.decl.signalKind);
    if (d.decl.range)
        bits.push(d.decl.range);
    if (d.decl.width !== null && d.decl.width !== undefined)
        bits.push(`${d.decl.width} bit`);
    return bits.length > 0 ? `  \`${bits.join(' ')}\`` : '';
}
/** 别名在源码里的位置（端口声明 / 内部信号声明 / 模块头） */
function aliasLoc(indexer, moduleName, net) {
    const mod = indexer.getModule(moduleName);
    if (!mod)
        return {};
    const p = mod.ports.find((x) => x.name === net);
    if (p)
        return { file: mod.file, line: p.line, offset: p.offset };
    const s = mod.signals.get(net);
    if (s)
        return { file: mod.file, line: s.line, offset: s.offset >= 0 ? s.offset : undefined };
...
/** 过长的行截断，避免一屏都是横向滚动 */
function clipLine(s, max = 200) {
    const t = s.replace(/\s+$/, '');
    return t.length > max ? `${t.slice(0, max - 1)}…` : t;
}
/** 把源码片段包成带语言标记的代码块（编辑器会做语法高亮） */
function codeBlock(source, file) {
    const body = source
        .split('\n')
        .map((l) => clipLine(l))
        .join('\n');
...
/** 一节引用列表（驱动源 / 负载 / 上级连接） */
function refSection(title, hint, refs, fallbackName, maxRefs, emptyNote) {
    const shown = refs.slice(0, maxRefs);
    const lines = [`**${title}** (${refs.length}) — ${hint}\n`];
    if (refs.length === 0) {
 ...
 * 这样信息框不会出现"半截图"式的残缺。
 */
function renderSignalHoverMarkdown(indexer, d, name, opts = {}, 
/** 声明处的原始源码（含上方紧邻的注释行），由调用方读取后传进来 */
declSource) {
    const sections = opts.sections && opts.sections.length > 0 ? opts.sections : exports.DEFAULT_HOVER_OPTIONS.sections;
    const maxRefs = Math.max(1, opts.maxRefs ?? exports.DEFAULT_HOVER_OPTIONS.maxRefs);
    const maxLines = Math.max(4, opts.maxLines ?? exports.DEFAULT_HOVER_OPTIONS.maxLines);
    const out = [];
    let used = 0;
    let truncated = false;
    ...
    for (const section of sections) {
        ...
        switch (section) {
            case 'decl': {
                const declLink = d.decl.file && d.decl.line !== undefined
                    ? locLink(d.decl.file, d.decl.line, d.decl.offset, name)
                    : '';
                if (d.decl.kind === 'submodule-port') {
                    push(`模块 \`${d.decl.subModule}\` 的端口（实例 \`${d.decl.instanceName}\`）` +
                        (declLink ? ` · 声明于 ${declLink}` : '') +
                        `\n\n`);
                }
                else {
                    push(`\`${d.moduleName}\` 中的 ${d.decl.kind === 'port' ? '端口' : '内部信号'}` +
                        (declLink ? ` · 声明于 ${declLink}` : '') +
                        `\n\n`);
                }
            ...
            case 'drivers':
                push(refSection('驱动源', '在哪个模块产生', innerDrivers, name, maxRefs, '_未发现直接驱动源_'));
                break;
            case 'loads':
            ...
            case 'parent':
                ...
                break;
            case 'path': {
                const paths = (0, hierarchy_1.hierarchyPaths)(indexer, d.moduleName, name, { maxPaths: Math.min(maxRefs, 6) });
                const prim = (0, hierarchy_1.primaryPath)(paths);
                if (!prim)
                    break;
                const extra = paths.length > 1 ? `（共 ${paths.length} 条可能的层次路径）` : '';
                const lines = [`**层次路径** ${extra}`];
