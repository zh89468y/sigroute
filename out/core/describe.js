...
exports.describeSignal = describeSignal;
exports.moduleLocation = moduleLocation;
exports.locText = locText;
/**
 * 单节最多收集多少条引用。
 *
...
function connPort(indexer, instType, c) {
    if (c.port)
        return c.port;
    if (c.portIndex === null)
        return null;
...
function describeSignal(indexer, mod, net) {
    const drivers = [];
    const loads = [];
    // ---------------------------------------------------------------- 声明
    let decl = { kind: 'unknown', width: null, range: null };
    let dir = null;
    ...
    if (dir === 'input' || dir === 'inout') {
        for (const site of indexer.getInstantiations(mod.name)) {
            const c = site.instance.connections.find((x) => connPort(indexer, site.instance.moduleType, x) === net);
            if (!c)
                continue;
    ...
    for (const ab of mod.alwaysBlocks) {
        ...
        if (!hit && !ab.readNets.includes(net))
            continue;
        const sens = ab.edgeNets.slice(0, 3).join(', ');
        loads.push({
            kind: 'proc-read',
...
function findSubmodulePort(indexer, mod, net) {
    for (const inst of mod.instances) {
        for (const c of inst.connections) {
            const pname = connPort(indexer, inst.moduleType, c);
            if (pname !== net)
...
/** 模块定义位置（供"跳到模块定义"用） */
function moduleLocation(indexer, moduleName) {
