...
exports.mergeParallelEdges = mergeParallelEdges;
exports.filterGraphByHops = filterGraphByHops;
exports.computeHierarchyFrames = computeHierarchyFrames;
exports.layoutSignalGraph = layoutSignalGraph;
const DEFAULTS = {
    direction: 'LR',
    nodeWidth: 196,
    nodeHeight: 64,
 ...
 * 信息不丢，图面清晰。
 */
function mergeParallelEdges(graph) {
    const map = new Map();
    let mergedAny = false;
    for (const e of graph.edges) {
        ...
        if (e.renamed && !cur.renamed) {
            ...
            if (!cur.flags.includes('renamed'))
                cur.flags.push('renamed');
        }
    }
    if (!mergedAny)
        return graph;
...
function computeHierarchyFrames(graph, layout, padding = 12) {
    const pos = new Map(layout.nodes.map((n) => [n.id, n]));
    const buckets = new Map();
    for (const n of graph.nodes) {
        if (n.isStart)
            continue;
        const pairs = n.instances && n.instances.length > 0
            ? n.instances
    ...
    for (const bucket of buckets.values()) {
        ...
        if (!contiguous)
            continue; // 中间夹着别的节点，框出来反而更乱
        const ids = [...new Set(bucket.items.map((i) => i.id))];
        const boxes = ids.map((id) => pos.get(id)).filter((b) => !!b);
        if (boxes.length === 0)
            continue;
...
function layoutSignalGraph(graph, options = {}) {
