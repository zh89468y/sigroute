...
const hierarchy_1 = require("./core/hierarchy");
const graph_1 = require("./core/graph");
const indexer_1 = require("./core/indexer");
const codeLens_1 = require("./vscode/codeLens");
const export_1 = require("./vscode/export");
const fsProvider_1 = require("./vscode/fsProvider");
const graphPanel_1 = require("./vscode/graphPanel");
const mindmapView_1 = require("./vscode/mindmapView");
const providers_1 = require("./vscode/providers");
const traceTree_1 = require("./vscode/traceTree");
let indexer;
let treeProvider;
let treeView;
/**
...
function activate(context) {
    ...
    indexer = new indexer_1.WorkspaceIndexer();
    treeProvider = new traceTree_1.TraceTreeProvider();
    statusBar = vscode.window.createStatusBarItem(vscode.StatusBarAlignment.Right, 100);
    statusBar.command = 'sigroute.rebuildIndex';
    updateStatus();
    // 视图模式：树视图 / 脑图 二选一（用 when 子句切换，面板位置不变）
    const savedMode = context.globalState.get('sigroute.viewMode', 'tree');
    ...
    mindmap = new mindmapView_1.MindmapViewProvider(treeProvider, {
    ...
    context.subscriptions.push(vscode.window.registerWebviewViewProvider(mindmapView_1.MindmapViewProvider.viewType, mindmap), mindmap);
    // 视图模式：树视图 / 脑图 二选一（用 when 子句切换，面板位置不变）
    void vscode.commands.executeCommand('setContext', 'sigroute.filter', 'all');
    void vscode.commands.executeCommand('setContext', 'sigroute.hasResult', false);
    treeProvider.setExpandHandler(() => void rerunTrace());
    const reg = (id, handler) => context.subscriptions.push(vscode.commands.registerCommand(id, handler));
    ...
    reg('sigroute.traceAt', (file, offset, name) => traceFromHover(file, offset, name));
    reg('sigroute.traceFromName', () => traceFromName());
    reg('sigroute.tracePort', (moduleName) => traceFromPort(String(moduleName)));
    ...
    reg('sigroute.showModuleInfoFor', (moduleName) => showModuleInfo(String(moduleName)));
    reg('sigroute.reveal', (file, line, offset) => revealLocation(file, line, offset));
    // ---- 新增：层次路径 / 别名 / 过滤 / 展开 / 视图切换 ----
    reg('sigroute.copyPath', (text) => copyHierarchyPath(text ? String(text) : undefined));
    reg('sigroute.findAliases', () => findAliases());
    reg('sigroute.expandChildren', (key) => treeProvider.expand(String(key)));
    reg('sigroute.cycleFilter', () => cycleFilter());
    reg('sigroute.filterSet', (value) => applyFilter(normalizeFilter(value)));
    ...
    context.subscriptions.push(vscode.workspace.onDidSaveTextDocument((doc) => {
        if (!isVerilogDoc(doc))
            return;
        if (indexer.current.stats.fileCount === 0)
            return; // 尚未索引
    ...
    codeLens = new codeLens_1.SigCodeLensProvider(providerCtx);
    context.subscriptions.push(vscode.languages.registerHoverProvider(providers_1.VERILOG_SELECTOR, new providers_1.SigHoverProvider(providerCtx)), vscode.languages.registerDefinitionProvider(providers_1.VERILOG_SELECTOR, new providers_1.SigDefinitionProvider(providerCtx)), vscode.languages.registerReferenceProvider(providers_1.VERILOG_SELECTOR, new providers_1.SigReferenceProvider(providerCtx)), vscode.languages.registerDocumentHighlightProvider(providers_1.VERILOG_SELECTOR, new providers_1.SigDocumentHighlightProvider()), vscode.languages.registerCodeLensProvider(providers_1.VERILOG_SELECTOR, codeLens));
    ...
    context.subscriptions.push(vscode.workspace.onDidChangeConfiguration((e) => {
        if (!e.affectsConfiguration('sigroute'))
            return;
        codeLens.refresh();
...
async function toggleViewMode() {
    const cur = extensionCtx.globalState.get('sigroute.viewMode', 'tree');
    ...
    vscode.window.setStatusBarMessage(next === 'mindmap'
        ? 'SigRoute：已切换到脑图视图（再点一次切回树视图）'
        : 'SigRoute：已切换到树视图（再点一次切回脑图）', 4000);
}
// ------------------------------------------------------------------ 定义跳转策略
/**
 ...
 */
async function configureDirectGoto() {
    const cfg = vscode.workspace.getConfiguration('editor');
    const key = 'gotoLocation.multipleDefinitions';
    const cur = cfg.get(key, 'peek');
    if (cur === 'goto') {
        const pick = await vscode.window.showInformationMessage('SigRoute：Ctrl+点击 / F12 已经是直接跳转（editor.gotoLocation.multipleDefinitions = goto）。', '恢复为预览列表');
        if (pick === '恢复为预览列表') {
    ...
    const ok = await vscode.window.showWarningMessage('当有多个「转到定义」提供者时（本工程里还装着别的 Verilog 扩展），' +
        'VSCode 默认弹出候选预览窗而不是直接跳过去。改成直接跳转到第一个结果吗？\n\n' +
        '这会修改 VSCode 全局设置 editor.gotoLocation.multipleDefinitions = goto（影响所有语言），之后可随时恢复。', { modal: true }, '改为直接跳转');
    if (ok !== '改为直接跳转')
        return;
    await cfg.update(key, 'goto', vscode.ConfigurationTarget.Global);
    const undo = await vscode.window.showInformationMessage('已改为直接跳转：Ctrl+点击 / F12 会一步跳到第一个定义，不再弹候选列表。', '恢复为预览列表');
    if (undo === '恢复为预览列表') {
        await cfg.update(key, undefined, vscode.ConfigurationTarget.Global);
        void vscode.window.showInformationMessage('已恢复：多处定义时仍弹预览列表。');
    }
...
function cycleFilter() {
    ...
    const next = cur === 'all' ? 'cross' : cur === 'cross' ? 'renamed' : 'all';
    applyFilter(next);
    const text = next === 'all'
        ? 'SigRoute：显示全部节点'
        : next === 'cross'
...
function updateTreeChrome() {
    ...
    try {
        ...
        const bits = [];
        if (filter !== 'all')
            bits.push(filter === 'cross' ? '只看跨模块' : '只看改名');
        bits.push(`${shown} 个节点`);
...
function countVisible(node, childrenOf) {
    if (!node)
        return 0;
    let n = 1;
    for (const c of childrenOf(node))
        n += countVisible(c, childrenOf);
    return n;
}
...
function readConfig() {
    ...
    return {
        includeGlobs: cfg.get('includeGlobs', ['**/*.v', '**/*.sv', '**/*.vh', '**/*.svh']),
        ...
        auxGlobs: cfg.get('blackboxSources', ['**/*.veo', '**/*.vho']),
        honorIfdef: cfg.get('honorIfdef', true),
        inlineIncludes: cfg.get('inlineIncludes', true),
        maxInstancesPerModule: cfg.get('maxInstancesPerModule', 200),
        defines: cfg.get('defines', []),
        undefines: cfg.get('undefines', []),
        useCache: cfg.get('indexCache', true),
    };
}
function getGlobs(c = readConfig()) {
...
async function buildIndex() {
    ...
    const provider = new fsProvider_1.VscodeFileProvider(() => getGlobs(c));
    ...
    if (c.useCache) {
        ...
        if (restored) {
            ...
            vscode.window.setStatusBarMessage(`SigRoute: 索引缓存命中（${s.moduleCount} 个模块 / ${s.instanceCount} 个例化，跳过解析）`, 5000);
            updateTreeChrome();
            return;
        }
    }
    // ---- 2) 全量解析 ----
    await vscode.window.withProgress({ location: vscode.ProgressLocation.Window, title: 'SigRoute 建立索引' }, async (progress) => {
    ...
    vscode.window.setStatusBarMessage(`SigRoute: 已索引 ${s.moduleCount} 个模块 / ${s.instanceCount} 个例化（${s.parseMs}ms）` +
        (extra.length > 0 ? `｜${extra.join('｜')}` : ''), 6000);
    if (c.useCache)
        void saveCache(provider, c).catch(() => undefined);
    updateTreeChrome();
}
// ---- 索引缓存 ----
function cacheFileUri() {
...
function fingerprintOf(c) {
    return (0, cache_1.optionsFingerprint)({
        honorIfdef: c.honorIfdef,
        inlineIncludes: c.inlineIncludes,
        maxInstancesPerModule: c.maxInstancesPerModule,
        extraDefines: c.defines,
...
async function stampFiles(provider, files) {
    if (!provider.stat)
        return null;
    const out = [];
    const BATCH = 64;
    for (let i = 0; i < files.length; i += BATCH) {
        ...
        for (let k = 0; k < slice.length; k++) {
            const s = stats[k];
            if (!s)
                return null; // 有文件读不到就不敢用缓存
            out.push({ path: slice[k].replace(/\\/g, '/'), mtimeMs: s.mtimeMs, size: s.size });
        }
    }
    return out;
}
async function tryLoadCache(provider, c) {
    try {
        ...
        if (!raw || raw.version !== cache_1.CACHE_VERSION)
            return false;
        if (raw.options !== fingerprintOf(c))
            return false;
        const files = (await provider.listFiles()).map((f) => f.replace(/\\/g, '/'));
        const stamps = await stampFiles(provider, files);
        if (!stamps || !(0, cache_1.sameFileSet)(raw.files ?? [], stamps))
            return false;
        const index = (0, cache_1.deserializeIndex)(raw.index);
        if (!index)
            return false;
...
async function saveCache(provider, c) {
    const files = (await provider.listFiles()).map((f) => f.replace(/\\/g, '/'));
    const stamps = await stampFiles(provider, files);
    if (!stamps)
        return;
    const payload = {
        version: cache_1.CACHE_VERSION,
        options: fingerprintOf(c),
        files: stamps,
        includeTexts: Object.fromEntries(indexer.cachedIncludeTexts),
        index: (0, cache_1.serializeIndex)(indexer.current),
    };
...
async function rebuildIndex(showMessage) {
    indexPromise = null;
    await ensureIndex();
    if (showMessage) {
        const s = indexer.current.stats;
        ...
        void vscode.window.showInformationMessage(`SigRoute 索引完成：${s.fileCount} 个文件、${s.moduleCount} 个模块、${s.instanceCount} 个例化，耗时 ${s.parseMs}ms。` +
...
function updateStatus() {
    ...
    if (s.fileCount === 0) {
        ...
        statusBar.tooltip = new vscode.MarkdownString(`**SigRoute 索引**\n\n- 文件：${s.fileCount}\n- 模块：${s.moduleCount}\n- 例化：${s.instanceCount}\n- 网络等价类：${nets.classes}（其中 ${nets.aliasedNets} 个网络有别名）\n- 耗时：${s.parseMs}ms\n\n点击重建索引`);
    }
    statusBar.show();
}
// ------------------------------------------------------------------ 追踪
 ...
 */
async function runTrace(startModule, net, direction, openGraph, startAt) {
    const sp = (0, graph_1.resolveStartPoint)(indexer, startModule, net);
    const cfg = vscode.workspace.getConfiguration('sigroute');
    ...
    const result = (0, graph_1.traceSignal)(indexer, sp.module, sp.net, {
        direction,
        maxDepth: cfg.get('maxDepth', 30),
        maxNodes: cfg.get('maxNodes', 600),
        maxChildren: cfg.get('maxChildren', 24),
    ...
    updateTreeChrome();
    if (openGraph || cfg.get('autoShowGraph', false)) {
        graphPanel_1.SignalGraphPanel.show(extensionCtx.extensionUri, result.graph, graphDirection(), graphHops(), graphHandlers());
    }
    await focusResultView();
    mindmap.refresh();
    const scope = direction === 'both' ? '上游+下游' : direction === 'up' ? '上游' : '下游';
    vscode.window.setStatusBarMessage(`SigRoute: ${net}（${scope}）—— ${result.stats.nodes} 个节点，${result.modules.length} 个模块，` +
        `${result.stats.renames} 处改名${result.stats.uncertainties > 0 ? `，${result.stats.uncertainties} 处不确定` : ''}` +
        (sp.note ? ` ｜ ${sp.note}` : ''), 8000);
}
/** 把结果推到眼前：当前是树视图还是脑图 */
async function focusResultView() {
    const mode = extensionCtx.globalState.get('sigroute.viewMode', 'tree');
    if (mode === 'mindmap') {
        mindmap.refresh();
    ...
    return {
        revealTree: (kind, id) => {
            const doReveal = () => {
                ...
                if (!node) {
                    vscode.window.setStatusBarMessage(`SigRoute: 树视图里没有 ${id} 对应的节点（链路可能已被跳数裁剪）`, 4000);
                    return;
                }
                try {
                    void treeView
        ...
        traceNet: (net, moduleName) => {
            ...
            const edge = treeProvider.current?.graph.edges.find((e) => e.file && (e.fromNet === net || e.toNet === net));
...
async function saveSvg(svg, name) {
...
function moduleAtLocation(file, offset) {
    let mod = indexer.findModuleAt(file, offset);
    if (!mod) {
        const names = indexer.current.fileModules.get(file.replace(/\\/g, '/')) ?? [];
        if (names.length === 1)
            mod = indexer.getModule(names[0]);
    }
    return mod;
}
async function doTrace(direction, openGraph = false) {
    ...
    const target = (0, providers_1.wordAt)(doc, editor.selection.active);
    if (!target) {
    ...
    if (!mod) {
        void vscode.window.showWarningMessage(`未能在 ${doc.uri.path.split('/').pop()} 中定位模块（光标可能位于模块之外）。`);
        return;
    }
 ...
 */
async function traceFromHover(file, offset, name, openGraph = false) {
    await ensureIndex();
...
async function gotoParentConn() {
    ...
    const desc = (0, describe_1.describeSignal)(indexer, mod, target.name);
    ...
    const pick = await vscode.window.showQuickPick(outer.map((r) => ({
        label: r.text || '(未连接)',
        description: r.detail ?? '',
        detail: `${(0, describe_1.locText)(r.file, r.line)}`,
        ref: r,
...
async function copyHierarchyPath(text) {
    if (text) {
    ...
    const paths = (0, hierarchy_1.hierarchyPaths)(indexer, mod.name, target.name, { maxPaths: 12 });
    if (paths.length === 0) {
    ...
    const pick = await vscode.window.showQuickPick(paths.map((p) => ({
        label: p.text,
...
async function findAliases() {
    ...
    if (members.length <= 1) {
        void vscode.window.showInformationMessage(`${target.name} 没有发现改名：在索引范围内它只出现在 ${mod.name}（可能是黑盒 IP 的另一侧，或端口未连接）。`);
        return;
    }
    const pick = await vscode.window.showQuickPick(members.map((m) => {
        ...
        const where = targetMod ? (0, describe_1.locText)(targetMod.file, line) : '';
        return {
...
// ------------------------------------------------------------------ 端口追踪
async function traceFromPort(moduleName) {
    await ensureIndex();
    ...
    const pick = await vscode.window.showQuickPick(mod.ports.map((p) => ({
        label: p.name,
        description: `${p.direction ?? '?'}${p.msb !== null && p.lsb !== null ? ` [${p.msb}:${p.lsb}]` : ''}${p.width !== null ? ` · ${p.width} bit` : ''}`,
        detail: (0, describe_1.locText)(mod.file, p.line),
        port: p,
    })), {
...
function graphDirection() {
    const v = vscode.workspace.getConfiguration('sigroute').get('graphDirection', 'LR');
    return v === 'TD' ? 'TD' : 'LR';
}
/** 框图默认跳数：0 = 全部 */
function graphHops() {
    const v = vscode.workspace.getConfiguration('sigroute').get('graphHops', 1);
...
function showGraph() {
    ...
    if (result.graph.nodes.length <= 1) {
        void vscode.window.showWarningMessage(`信号 ${result.start.netName} 没有跨模块连接，无法生成框图（可查看树视图了解模块内情况）。`);
        return;
    }
    graphPanel_1.SignalGraphPanel.show(extensionCtx.extensionUri, result.graph, graphDirection(), graphHops(), graphHandlers());
}
async function traceFromName() {
    const editor = vscode.window.activeTextEditor;
    const maxDepth = vscode.workspace.getConfiguration('sigroute').get('maxDepth', 30);
    await ensureIndex();
    const startModule = editor
        ? indexer.findModuleAt(editor.document.uri.fsPath, editor.document.offsetAt(editor.selection.active))
        : undefined;
    const seed = editor
        ? (0, providers_1.wordAt)(editor.document, editor.selection.active)?.name ?? ''
        : '';
    ...
    if (!start)
        return;
    const sp = (0, graph_1.resolveStartPoint)(indexer, start, name);
    const cfg = vscode.workspace.getConfiguration('sigroute');
    // 手输的名字如果就是光标下那个词，根节点同样跳回这里
    let startAt;
    if (editor) {
        const cur = (0, providers_1.wordAt)(editor.document, editor.selection.active);
 ...
 */
async function revealLocation(file, line, offset, label) {
    try {
        const doc = await vscode.workspace.openTextDocument(vscode.Uri.file(file));
        ...
        void vscode.window.showErrorMessage(`无法打开 ${file}：${err.message}`);
    }
}
 ...
 */
function resolveRevealRange(doc, line, offset, label) {
    // 1) 精确偏移：从该位置扩展出完整标识符
    ...
    if (name) {
        ...
        if (idx >= 0)
            return new vscode.Range(ln, idx, ln, idx + name.length);
    }
...
function indexOfIdentifier(text, name) {
...
async function exportMermaid() {
    ...
    const cfg = vscode.workspace.getConfiguration('sigroute');
    const text = (0, export_1.toMermaid)(result, { direction: cfg.get('mermaidDirection', 'LR') });
    await vscode.env.clipboard.writeText(text);
    const doc = await vscode.workspace.openTextDocument({ content: text, language: 'markdown' });
    await vscode.window.showTextDocument(doc, { preview: false });
    void vscode.window.showInformationMessage('Mermaid 图已复制到剪贴板，可粘贴到 mermaid.live、Typora 或 Markdown 预览中查看。');
}
async function exportMarkdown() {
    ...
    const text = (0, export_1.toMarkdown)(result);
    const target = await vscode.window.showQuickPick([
        { label: '$(clippy) 复制到剪贴板', value: 'clipboard' },
        { label: '$(file-add) 在新文档中打开', value: 'document' },
...
async function showModuleInfo(moduleName) {
    ...
    if (mod.declOnly)
        lines.push('- 来源：IP 例化模板（仅用于补齐端口方向）');
    lines.push('');
    lines.push('## 端口');
    lines.push('');
    ...
    for (const p of mod.ports) {
        lines.push(`| ${p.index + 1} | \`${p.name}\` | ${p.direction ?? '?'} | ${p.width ?? '?'} | ${p.msb !== null && p.lsb !== null ? `[${p.msb}:${p.lsb}]` : '-'} |`);
    }
    lines.push('');
    lines.push('## 例化的子模块');
    lines.push('');
    ...
    for (const inst of mod.instances) {
        const indexed = indexer.hasModule(inst.moduleType)
            ? indexer.isDeclOnly(inst.moduleType)
                ...
                : '是'
            : '**否（IP/黑盒）**';
        lines.push(`| \`${inst.instanceName}\` | \`${inst.moduleType}\` | ${inst.connections.length} | ${inst.line + 1}${inst.inGenerate ? ' (generate)' : ''} | ${indexed} |`);
    }
    lines.push('');
    lines.push('## 被例化位置');
    lines.push('');
    if (sites.length === 0) {
        ...
        for (const s of sites) {
            lines.push(`- \`${s.parentModule}\` 中的 \`${s.instance.instanceName}\` — ${s.parentFile.split(/[\\/]/).pop()}:${s.instance.line + 1}`);
        }
    }
...
// ------------------------------------------------------------------ 工具
function isVerilogDoc(doc) {
    const ids = ['verilog', 'systemverilog', 'verilog-hdl'];
    if (ids.includes(doc.languageId))
        return true;
